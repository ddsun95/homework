...
\begin{document}
	\begin{flushleft}
		...
		\end{bmatrix}
		\]
		Since \emph{M'} is a $2^{n + 1} \times 2^{n + 1}$ matrix, \emph{M'} can be evenly divided into four quadrants with each quadrant containing $2^n \times 2^n$ matrix entries. This means that the initial 0 entry in \emph{M'} must be within one of the four quadrants.
